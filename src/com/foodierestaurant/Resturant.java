...
public class Resturant {
	...
	public void startResurant() {
		...
		int choosenTable= allocateTable(customerObj,persons); // calling allocateTable method to allocate suitable table
		serveWater(choosenTable);							// Serving water on the table
		System.out.println(name+ ",Please tell your order ?? \n \n");
		menu.displayMenu();
		int yourOrderID= new Orders().takeOrder(menu);				 	// Calling the takeOrder method from Order class to take order from customer
		...
		if(feedbackDecision.equalsIgnoreCase("Y")) {
			...
			customerObj.setFeedback(feedback);
		}
		else if (feedbackDecision.equalsIgnoreCase("Y")) {
